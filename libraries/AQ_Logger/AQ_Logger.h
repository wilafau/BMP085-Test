...
#ifndef _AEROQUAD_LOGGER_H_
#define _AEROQUAD_LOGGER_H_

//  This class is used with the OpenLog serial logger from Sparkfun
//	to implement a flexible on-board black box with these features:
//		. At flight commencement a uniquely numbered file is created
//		  on the OpenLog SD card.  The file will contain a series of
//        binary data records.
//		. A special header record is written to the file first which 
//        describes the format of each type of data record including
//		. each parameter type & size.
//      . There are generic parameter types, such as float or int and
//        special compressed types which must be reconstructed on the
//        host side program. 

//		. 
//		. Each record has a start byte and an ending checksum.
//     .  Data is written in a compressed binary form for efficiency
//     .  Data is written as records containing telementry data
//     .  Different record types can be intermixed. 
//     .  A "header" record is 
...
#endif  // #define _AEROQUAD_LOGGER_H_
